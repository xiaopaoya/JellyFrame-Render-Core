    ...
    while (!pending.empty()) {
        ...
        if (current->local_dirty_flags != DomDirtyNone) {
            if (current->type != NodeType::Text ||
                ...
                !dirty_flags_are_text_layout_only(current->local_dirty_flags)) {
                return false;
            }
...
std::size_t validate_dirty_layout_boxes(const LayoutBox& layout,
...
bool text_dirty_can_reuse_layout(const Node& document,
    ...
    if ((document.dirty_flags & DomDirtyText) == 0U ||
        (document.dirty_flags & (DomDirtyTree | DomDirtyAttributes | DomDirtyStyle)) != 0U ||
    ...
    const std::size_t matched = validate_dirty_layout_boxes(layout, dirty_text_nodes, text_measure);
    return matched == dirty_text_nodes.size();
}
