...
- `overflow: hidden`、`overflow: clip`、`overflow: auto` 和 `overflow: scroll`
  ...
  `scroll_y` offset；后代绘制和命中测试会在既有 clip 内移动。Win32 宿主会把 wheel、arrow 和 pointer drag 默认动作映射为这些 offset，在允许 present 时追加有界惯性，并在面积预算内把容器视口作为 dirty rect 重绘。滚动条、水平滚动和 strip 级 scroll blit 不在这一切片内。
