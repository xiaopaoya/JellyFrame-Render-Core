...
- `transform: translate()/scale()` 子集创建 composited layer，并由软件合成器执行整数近似的 2D 平移和中心缩放。
...
- 当前 transform-origin 固定近似为 layer bounds 中心；不支持 rotate/skew/matrix/perspective。
- 已检测圆角裁剪，但当前近似为矩形裁剪。
...
- 完整 Transform 矩阵、transform-origin 和变换后的精确裁剪。
