...
struct FrameScratch {
    ...
    void reserve_from_budgets(const HostBudgets& budgets) {
        ...
        animation_dirty_region.rects.reserve(max_dirty_rects);
    ...
    void begin_frame() {
        clear_dirty_region();
        animation_dirty_region.rects.clear();
        animation_dirty_region.mode = DirtyRegionMode::Clean;
        animation_dirty_region.fallback_reason = DirtyRegionFallbackReason::None;
    ...
    void release() {
        ...
        std::vector<Rect>().swap(animation_dirty_region.rects);
