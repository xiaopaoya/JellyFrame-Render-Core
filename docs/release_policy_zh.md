...
首个带签名的 `v0.6.0` release 建立了确定性 source archive 和 SHA-256 sidecar；Runtime 当前接纳带签名的 `v0.6.2`。独立仓库 `master` 已同步 JellyFrame `b1196f67` 之前的 release 后开发更新，但该开发头不是 Runtime dependency。这不表示许可证、贡献者政策或开源意向的改变。
